...
def main(cfg: DictConfig):
    ...
    logger.info("reading data source: %s", "dpr_wiki")

    ctx_src = hydra.utils.instantiate(cfg.ctx_sources["dpr_wiki"])
    all_passages_dict = {}
    ctx_src.load_data_to(all_passages_dict)
    all_passages = [(k, v) for k, v in all_passages_dict.items()]

    shard_size = math.ceil(len(all_passages) / 1)
    ...
    file = "/anaconda3/envs/paper/DPR/output/DenseEmbedding/SimCSE_embedding" + "_" + str(0)
